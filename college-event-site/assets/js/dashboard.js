...
class DashboardManager {
    ...
    setupUserInterface() {
        ...
        if (profileRoleEl) profileRoleEl.textContent = this.formatRole(this.user.role);
        ...
        // Update avatar
        const avatarElements = document.querySelectorAll('.avatar-img, .profile-img');
    ...
    calculateStatistics() {
        ...
        return {
            ...
            eventsAttended: this.registeredEvents.filter(event => 
                new Date(event.date) < now && event.status !== 'cancelled'
            ).length,
            upcomingEvents: this.registeredEvents.filter(event => 
                new Date(event.date) > now
            ).length,
            organizedEvents: this.organizedEvents.length,
            totalRegistrations: this.organizedEvents.reduce((total, event) => 
                total + (event.attendees || 0), 0
            )
        };
    }
