...
export class AdminManager {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (action === "delete") {
          ...
          users[userIndex].statusChangedBy =
  ...
  updateUserRowStatus(userId, newStatus) {
    ...
    const banButton = row.querySelector(
    ...
    if (banButton) {
      if (newStatus === "banned") {
        ...
        `;
        banButton.className =
          "action-btn px-3 py-1 border border-green-500 text-green-600 hover:bg-green-50 rounded text-sm";
      } else {
        ...
        `;
        banButton.className =
          "action-btn px-3 py-1 border border-orange-500 text-orange-600 hover:bg-orange-50 rounded text-sm";
      }
    }
  }
  ...
  performSystemCheck() {
    ...
    const results = Object.entries(checks)
      .map(([check, status]) => `${check}: ${status ? "✅ OK" : "❌ FAIL"}`)
      .join("\n");
